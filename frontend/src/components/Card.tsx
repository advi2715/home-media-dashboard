...
export function Card({ children, className = "", title, href, action, noPadding = false }: CardProps) {
    return (
                ...
                <div className="px-5 py-4 flex justify-between items-center border-b border-white/5 bg-white/2">
                    {title && (href ? (
                        <a 
                            ...
                            className="text-sm font-bold uppercase tracking-wider text-slate-400 hover:text-white transition-colors"
                        >
                            {title}
                        </a>
                    ) : (
